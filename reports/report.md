...
**The purest case is Claude Opus 4.8 — the sound recurs even though the token never does.** Section 6.3 recorded Opus 4.8 as the cohort's diversity leader, with *zero* repeated character names, which is why it is absent from the rows above. But re-examining its ten runs by name *shape* rather than exact token shows the V-surname pull fully intact: **eight of the ten runs independently coin a distinct V-initial surname** — `Vance`, `Vael`, `Voss`, `Veymar`, `Vendramin`, `Okonkwo-Vey`, `Vahn`, `Vesh` — no two alike, so each appears in only 1/10 runs. Opus therefore exhibits the cleanest possible separation of the two axes: token-level repetition of **zero**, phonetic recurrence of **8/10**. The model that most thoroughly avoids reusing a *name* still cannot avoid reusing the *sound* — which is the single strongest piece of evidence that this cluster is phonetic, not lexical, and that the "broken pattern" of Section 6.3 is broken only at the level of exact tokens.

(The phonetic-clustering metric implemented since — `utils/metrics/phonetic_names` — reproduces this 8/10 and was what surfaced one correction to the hand-count above: it recovered `Okonkwo-Vey`, a V-surname buried in a hyphenated compound that exact-token counting hides, and is exactly the case the metric exists to catch. It also illustrates its own caveat — a naive run reports 9/10 by mistaking the place "Veyra Colony" (a tenth run) for a person, the spaCy NER false-positive problem of Section 4.4. Filtering that, the genuine count is 8/10.)
